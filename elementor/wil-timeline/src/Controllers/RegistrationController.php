...
use WilokeTimeline\Controllers\PostControl\CustomPostControl;
use WilokeTimeline\Controllers\SliderControl\CustomSliderControl;
...
//	}
	public function registerScripts()
	{
		...
		$aHandleJs=[];
		wp_register_style(
			WILOKE_WILOKETIMELINE_NAMESPACE . md5(App::get('dataConfig')['css']),
			App::get('dataConfig')['css'],
			[],
			WILOKE_WILOKETIMELINE_VERSION);
		$aHandleCss[]=WILOKE_WILOKETIMELINE_NAMESPACE . md5(App::get('dataConfig')['css']);


		wp_register_script(
			WILOKE_WILOKETIMELINE_NAMESPACE . md5(App::get('dataConfig')['js']),
			App::get('dataConfig')['js'],
			['elementor-frontend'],
			WILOKE_WILOKETIMELINE_VERSION,
			true
		);
		$aHandleJs[]=WILOKE_WILOKETIMELINE_NAMESPACE . md5(App::get('dataConfig')['js']);
		if (isset(App::get('dataConfig')['libs']['css']) && !empty($aLibCss = App::get('dataConfig')['libs']['css'])) {
			foreach ($aLibCss as $urlCss) {
				wp_register_style(
					WILOKE_WILOKETIMELINE_NAMESPACE . md5($urlCss),
					$urlCss,
					[], WILOKE_WILOKETIMELINE_VERSION);
				$aHandleCss[]=WILOKE_WILOKETIMELINE_NAMESPACE . md5($urlCss);
			}
		}
		...
		if (isset(App::get('dataConfig')['libs']['js']) && !empty($aLibJs = App::get('dataConfig')['libs']['js'])) {
			foreach ($aLibJs as $urlJs) {
				wp_register_script(
					WILOKE_WILOKETIMELINE_NAMESPACE . md5($urlJs),
					$urlJs,
					[],
					WILOKE_WILOKETIMELINE_VERSION,
					true
			...
			$aHandleJs[]=WILOKE_WILOKETIMELINE_NAMESPACE . md5($urlJs);
		}
